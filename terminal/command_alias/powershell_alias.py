...
class PowerShellAliasManager(BaseAliasManager):
    def profile_path(self, prefer_ps7: Optional[bool] = None) -> Path:
        ...
        if prefer_ps7 is None:
            try:
                t = (get_terminal_type() or "").lower() # type: ignore
            except Exception:
                t = ""
            if t == "pwsh":
                prefer_ps7 = True
        ...
        try:
            ...
            for profile_path in profiles:
                ...
                if aggressive:
                    ...
                    alias_names = set() # type: ignore
                    target_names = set() # type: ignore
                    func_names = set() # type: ignore
                    ...
                    for match in marked_pattern.finditer(original_content):
                        ...
                        for alias_match in re.finditer(r'Set-Alias\s+(\S+)\s+(\S+)', block):
                            alias_names.add(alias_match.group(1)) # type: ignore
                            target_names.add(alias_match.group(2)) # type: ignore
                        for func_match in re.finditer(r'function\s+(\S+)\s*\{', block):
                            func_names.add(func_match.group(1)) # type: ignore
                    ...
                    for name in list(alias_names | target_names | func_names): # type: ignore
                        func_pat = re.compile(rf"function\s+{re.escape(name)}\s*{{[\s\S]*?}}", re.MULTILINE) # type: ignore
                        content = re.sub(func_pat, "", content)
                    
                    # 5) 过滤仅数字的噪声行和 Unicode 控制字符
                    lines = content.splitlines()
                    filtered_lines = []
                    cleanup_names = alias_names | target_names | func_names # type: ignore
                    ...
                    for line in lines:
                        ...
                        # 跳过仅包含清理名称的行
                        if cleanup_names and re.match(rf'^[\s\u200e\u200f\u202a-\u202e]*({"|".join(re.escape(name) for name in cleanup_names)})[\s\u200e\u200f\u202a-\u202e]*$', clean_line): # type: ignore
                            continue
                ...
                cleaned_profiles.append(str(profile_path)) # type: ignore
            
            if cleaned_profiles:
                return True, f"已清理 PowerShell 配置文件: {', '.join(cleaned_profiles)}" + (" 并进行了深度清理" if aggressive else "") # type: ignore
            else:
    ...
    def print_aliases(self) -> None:
        ...
        SUCCESS(profile_path) # type: ignore
        ...
        for m in marked_block_pat.finditer(content):
            ...
            if fn:
                func_names.add(fn.group(1)) # type: ignore
        ...
        if set_aliases or func_names:
            ...
            remaining_funcs = sorted(name for name in func_names if name not in alias_names) # type: ignore
            for name in remaining_funcs: # type: ignore
                SUCCESS(f"  {name}")
        else:
