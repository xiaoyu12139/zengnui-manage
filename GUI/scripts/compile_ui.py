...
def reset_ui_files():
    ...
    重置已编译的ui文件夹
    """
    ui_widget_path = CURRENT_DIR.parent / "src" / "ui_widget"
...
def create_ui_plugin():
    ...
    """
    ui_widget_path = CURRENT_DIR.parent / "src" / "ui_widget"
    if not ui_widget_path.exists():
        ui_widget_path.mkdir()
    ui_path = CURRENT_DIR.parent / "src" / "resources"
    for f in ui_path.iterdir():
        if f.is_dir() and f.name.endswith("_plugin"):
            (ui_widget_path / f"{f.name}").mkdir()
            (
                ui_widget_path / f"{f.name}" / "__init__.py"
            ).touch()  # 创建空的__init__.py文件
...
def main():
    ...
    for f in ui_path.rglob("*.ui"):
        if f.is_file() and f.name.endswith(".ui"):
            print(f"编译UI文件：{f}")
            ui_build_path = (
                ui_widget_path / f"{f.parent.parent.name}" / f"ui_{f.stem}.py"
            )
            ...
            open(init_path, "a").write(
                f"from .{ui_build_path.name.split('.')[0]} import Ui_{to_pascal_case(f.stem)}\n"
            )
    print("编译完成")
