...
class MainWindowCmdHandler:
    ...
    def assemble_cmd(self, vm_creator: callable):
        ...
        def activate_main_window():
            ...
            # 主窗口容器
            main_win_id = Global().views_manager.instance_view(
                str(hash(MainWindowView)), vm_creator()
            )
            # top 组件
            Global().command_manager.cmd(
                "d56194f5-4ab5-44d6-a65c-7d3161b7764a", main_win_id
            )
            # 诊断组件
            Global().command_manager.cmd(
                "8ba404a6-6cb6-4804-a719-c82163773eb2", main_win_id
            )
            # # status bar组件
            Global().command_manager.cmd(
                "cdec7d58-7551-411f-847b-6285e8eb52c2", main_win_id
            )
            # 激活菜单栏相关插件
