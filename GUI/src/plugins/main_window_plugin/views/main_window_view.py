...
class MainWindowView(QMainWindow):
    ...
    def __init__(self, parent: QWidget = None):
        ...
        self._resize_dir = (
            None  # 'left','right','top','bottom','tl','tr','bl','br'
        )
    ...
    def changeEvent(self, event):
        ...
        if event.type() == QEvent.ScreenChangeInternal:
            QTimer.singleShot(
                0,
                lambda: (self._apply_shadow(), self.ui.centralwidget.update()),
            )
    ...
    def create_vm_sig_connect(self):
        ...
        创建视图模型信号连接
        """
        self.view_model.sig_move_main_window.connect(
            self.on_sig_move_main_window
        )
        ...
        self.view_model.sig_diagnostics_main_window.connect(
            self.on_sig_diagnostics_main_window
        )
        self.view_model.sig_toggle_main_window_theme.connect(
            self.on_sig_toggle_main_window_theme
        )
        self.view_model.sig_register_menu_pane.connect(
            self.on_sig_register_menu_pane
        )
        self.view_model.sig_init_main_window.connect(
            self.on_sig_init_main_window
        )
        ...
        if not self.menu_list:
            if hasattr(menu_pane, "get_menu_name"):
                ...
                menu_item_widget = MenuItemWidget(
                    menu_name, pane_view_id, self.view_model._win_id
                )
